...
答案遮罩使用獨立 `answerMasks` 資料表與 `state.answerMasks`，以 `materialId`、頁碼及零到一的矩形座標保存，不重用問答圖釘座標。教材編輯頁可拖曳建立、點擊修改或刪除遮罩；課堂投影端與學生端預設遮住，點擊單一遮罩後只在目前裝置揭示，另有「全部遮回」；揭示狀態不進入 `classroomState`，避免學生裝置互相解鎖。批改彈窗不套用 PDF 遮罩。這是畫面遮罩，不宣稱能防止瀏覽器取得原始 PDF。
